...
with open('test_results.pkl', 'rb') as f:
    while True:
        try:
            ...
            row[1].text = tar_
            row[2].text = str("{:.2f}".format(keys_["accuracy"]))
            row[3].text = str("{:.2f}".format(keys_["specificity"]))
            row[4].text = str("{:.2f}".format(keys_["sensitivity"]))
            row[5].text = str("{:.2f}".format(keys_["auc"]))
        except EOFError:
                break 
